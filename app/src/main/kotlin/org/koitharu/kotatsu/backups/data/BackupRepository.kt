...
class BackupRepository @Inject constructor(
        ...
        for (section in BackupSection.entries) {
            when (section) {
                ...
                BackupSection.HISTORY -> output.writeJsonArray(
                    ...
                    data = database.getHistoryDao().dump().mapNotNull {
                        HistoryBackup(it).takeUnless { backup -> backup.manga.isBlocked() }
                    },
                    serializer = serializer(),
                )
                ...
                BackupSection.FAVOURITES -> output.writeJsonArray(
                    ...
                    data = database.getFavouritesDao().dump().mapNotNull {
                        FavouriteBackup(it).takeUnless { backup -> backup.manga.isBlocked() }
                    },
                    serializer = serializer(),
                )
                ...
                BackupSection.BOOKMARKS -> output.writeJsonArray(
                    ...
                    data = database.getBookmarksDao().dump().mapNotNull {
                        BookmarkBackup(it.first, it.second).takeUnless { backup -> backup.manga.isBlocked() }
                    },
                    serializer = serializer(),
                )
                ...
                BackupSection.SOURCES -> output.writeJsonArray(
                    ...
                    data = database.getSourcesDao().dumpEnabled().mapNotNull {
                        SourceBackup(it).takeUnless { backup -> blockedContentPolicy.isBlocked(backup.source) }
                    },
                    serializer = serializer(),
                )
                ...
                BackupSection.SCROBBLING -> output.writeJsonArray(
                    ...
                    data = database.getScrobblingDao().dumpEnabled().mapNotNull {
                        ScrobblingBackup(it).takeIf { backup -> canExportManga(backup.mangaId) }
                    },
                    serializer = serializer(),
                )
                ...
                BackupSection.STATS -> output.writeJsonArray(
                    ...
                    data = database.getStatsDao().dumpEnabled().mapNotNull {
                        StatisticBackup(it).takeIf { backup -> canExportManga(backup.mangaId) }
                    },
                    serializer = serializer(),
                )
    ...
    private fun dumpSettings(): String {
        val map = settings.sanitizeBlockedContentSettings(settings.getAllValues()).toMutableMap()
    ...
    private suspend fun canExportManga(mangaId: Long): Boolean {
        val manga = database.getMangaDao().find(mangaId)?.manga ?: return false
        return !blockedContentPolicy.isBlocked(manga.contentRating, manga.isNsfw, manga.source)
    }

    private fun MangaBackup.isBlocked(): Boolean {
        return blockedContentPolicy.isBlocked(contentRating, isNsfw, source)
    }
