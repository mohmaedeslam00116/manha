...
class DetailsLoadUseCase @Inject constructor(
	...
	operator fun invoke(intent: MangaIntent, force: Boolean): Flow<MangaDetails> = flow {
		val resolvedManga = requireNotNull(mangaDataRepository.resolveIntent(intent, withChapters = true)) {
			"Cannot resolve intent $intent"
		}
		val override = mangaDataRepository.getOverride(resolvedManga.id)
		val manga = resolvedManga.restrictIfBlocked(override?.contentRating)
		emit(
			MangaDetails(
				manga = manga,
				localManga = null,
		...
		if (blockedContentPolicy.isBlocked(manga.source, override?.contentRating ?: manga.contentRating)) {
			return@flow
		}
		if (manga.isLocal) {
	...
	private suspend fun FlowCollector<MangaDetails>.loadLocal(manga: Manga, override: MangaOverride?, force: Boolean) {
		...
		val localDetails = localMangaRepository.getDetails(manga).restrictIfBlocked()
		emit(
			MangaDetails(
				manga = localDetails,
				localManga = null,
		...
		if (blockedContentPolicy.isBlocked(localDetails)) {
			return
		}
		if (skipNetworkLoad) {
			return
	...
	private suspend fun getDetails(seed: Manga, force: Boolean) = runCatchingCancellable {
		...
		val cachingRepository = repository.asCachingRepository()
		if (cachingRepository != null) {
			cachingRepository.getDetails(seed, if (force) CachePolicy.WRITE_ONLY else CachePolicy.ENABLED)
		} else {
			repository.getDetails(seed)
		}.restrictIfBlocked()
	}.recoverNotNull { e ->
		if (e is NotFoundException) {
			recoverUseCase(seed)?.restrictIfBlocked()
		} else {
			null
		}
	}
	...
	private fun Manga.restrictIfBlocked(contentRatingOverride: org.koitharu.kotatsu.parsers.model.ContentRating? = null): Manga {
		return if (blockedContentPolicy.isBlocked(source, contentRatingOverride ?: contentRating)) {
			blockedContentPolicy.toRestricted(copy(contentRating = contentRatingOverride ?: contentRating))
		} else {
			this
		}
	}
