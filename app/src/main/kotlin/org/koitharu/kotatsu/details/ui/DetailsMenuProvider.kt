...
class DetailsMenuProvider(
	...
	override fun onPrepareMenu(menu: Menu) {
		...
		val isRestricted = viewModel.mangaDetails.value?.isRestricted == true
		menu.findItem(R.id.action_share).isVisible = !isRestricted && manga != null && AppRouter.isShareSupported(manga)
		menu.findItem(R.id.action_save).isVisible = !isRestricted && manga?.source != null && manga.source != LocalMangaSource
		menu.findItem(R.id.action_delete).isVisible = !isRestricted && manga?.source == LocalMangaSource
		menu.findItem(R.id.action_browser).isVisible = !isRestricted && manga?.publicUrl?.isHttpUrl() == true
		menu.findItem(R.id.action_alternatives).isVisible = !isRestricted && manga?.source != LocalMangaSource
		menu.findItem(R.id.action_shortcut).isVisible = !isRestricted && ShortcutManagerCompat.isRequestPinShortcutSupported(activity)
		menu.findItem(R.id.action_scrobbling).isVisible = !isRestricted && viewModel.isScrobblingAvailable
		menu.findItem(R.id.action_online).isVisible = !isRestricted && viewModel.remoteManga.value != null
		menu.findItem(R.id.action_stats).isVisible = !isRestricted && viewModel.isStatsAvailable.value
	}
