...
class AppSettings @Inject constructor(@ApplicationContext context: Context) {
	...
	var incognitoModeForNsfw: TriStateOption
		get() = TriStateOption.DISABLED
		set(value) = prefs.edit { putEnumValue(KEY_INCOGNITO_NSFW, TriStateOption.DISABLED) }
	...
	var blockedContentPolicyVersion: Int
		get() = prefs.getInt(KEY_BLOCKED_CONTENT_POLICY_VERSION, 0)
		set(value) = prefs.edit { putInt(KEY_BLOCKED_CONTENT_POLICY_VERSION, value) }
	...
	fun upsertAll(m: Map<String, *>) = prefs.edit {
		clear()
		putAll(sanitizeBlockedContentSettings(m))
	}

	fun enforceBlockedContentDefaults() = prefs.edit {
		putBoolean(KEY_DISABLE_NSFW, true)
		putBoolean(KEY_TRACKER_NO_NSFW, true)
		putBoolean(KEY_SUGGESTIONS_EXCLUDE_NSFW, true)
		putBoolean(KEY_DISCORD_RPC_SKIP_NSFW, true)
		putEnumValue(KEY_INCOGNITO_NSFW, TriStateOption.DISABLED)
	}

	fun sanitizeBlockedContentSettings(values: Map<String, *>): Map<String, *> = LinkedHashMap<String, Any?>(values.size + 5).apply {
		putAll(values)
		put(KEY_DISABLE_NSFW, true)
		put(KEY_TRACKER_NO_NSFW, true)
		put(KEY_SUGGESTIONS_EXCLUDE_NSFW, true)
		put(KEY_DISCORD_RPC_SKIP_NSFW, true)
		put(KEY_INCOGNITO_NSFW, TriStateOption.DISABLED.name)
	}
	...
	companion object {
		...
		const val KEY_BLOCKED_CONTENT_POLICY_VERSION = "blocked_content_policy_version"
