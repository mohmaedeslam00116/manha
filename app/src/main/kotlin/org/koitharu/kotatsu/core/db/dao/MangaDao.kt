...
abstract class MangaDao {
	...
	@Query(
		...
		WHERE source IN (:blockedSources)
			OR content_rating IN (:blockedRatings)
			OR (nsfw = 1 AND (content_rating IS NULL OR content_rating = ''))
		""",
	)
	abstract suspend fun findBlockedIds(
		blockedSources: Collection<String>,
		blockedRatings: Collection<String>,
	): LongArray
	...
	abstract suspend fun deleteBlocked(
		blockedSources: Collection<String>,
		blockedRatings: Collection<String>,
	): Int
