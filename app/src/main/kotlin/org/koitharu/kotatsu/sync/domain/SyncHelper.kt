...
class SyncHelper @AssistedInject constructor(
	...
	fun syncFavourites(stats: SyncStats) {
		...
		response?.favourites?.filterNot { it.manga.isBlocked() }?.let { favourites ->
	...
	fun syncHistory(stats: SyncStats) {
		...
		val request = Request.Builder()
			...
			.build()
		val response = httpClient.newCall(request).execute().parseDtoOrNull()
		response?.history?.filterNot { it.manga.isBlocked() }?.let { history ->
		...
		return provider.query(authorityHistory, TABLE_HISTORY).use { cursor ->
			...
			if (cursor.moveToFirst()) {
				do {
					...
					val manga = getManga(authorityHistory, mangaId)
					if (!manga.isBlocked()) {
						result.add(HistorySyncDto(cursor, manga))
					}
	...
	private fun getFavourites(): List<FavouriteSyncDto> {
		return provider.query(authorityFavourites, TABLE_FAVOURITES).map { cursor ->
			...
			if (manga.isBlocked()) null else FavouriteSyncDto(cursor, manga)
		}.filterNotNull()
	}
	...
	private fun MangaSyncDto.isBlocked(): Boolean {
		return blockedContentPolicy.isBlocked(contentRating, false, source)
	}
